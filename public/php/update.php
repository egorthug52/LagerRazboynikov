...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if (isset($_FILES['patient_file']) && $_FILES['patient_file']['error'] !== UPLOAD_ERR_NO_FILE) {
            ...
            $old_file_stmt = $conn->prepare("SELECT file_path FROM patient_files WHERE patient_id = :patient_id");
            $old_file_stmt->execute([':patient_id' => $id]);
            $old_file = $old_file_stmt->fetch(PDO::FETCH_ASSOC);
            if ($old_file && file_exists($old_file['file_path'])) {
                unlink($old_file['file_path']);
                $conn->prepare("DELETE FROM patient_files WHERE patient_id = :patient_id")->execute([':patient_id' => $id]);
            }
