$(document).ready(() => {
    ...
    const toggleFields = () => {
        ...
        confirmedDate.prop('disabled', isCancelled).attr('required', !isConfirmed && !isCancelled);
        cancellationDate.prop('disabled', isConfirmed).attr('required', !isConfirmed && !isCancelled);
    };
    ...
    const validateDates = () => {
        ...
        ['confirmed', 'cancellation'].forEach(type => {
            ...
            if (dateField.val() && new Date(dateField.val()).getTime() < diseaseTime) {
                alert(`Дата ${type === 'confirmed' ? 'подтверждения' : 'отмены'} не может быть раньше даты установления диагноза!`);
                dateField.val('');
                toggleFields();
            }
        });
    ...
    // Обработка полей имени, фамилии и отчества
    ['first_name', 'middle_name', 'last_name'].forEach(field => {
        $(`#${field}`).on('input', function() {
            let value = $(this).val().replace(/[^a-zA-Zа-яА-Я\s]/g, '');
